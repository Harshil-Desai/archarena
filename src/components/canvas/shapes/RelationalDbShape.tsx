...
export type RelationalDbShapeType = TLShape<"relationalDb">;

function LabelEditor({ shape }: { shape: RelationalDbShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-blue-500 rounded-lg
                 outline-none z-10"
...
export class RelationalDbShapeUtil extends ShapeUtil<RelationalDbShapeType> {
  static override type = "relationalDb" as const;

  getDefaultProps(): RelationalDbShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "postgresql", category: "storage", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: RelationalDbShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: RelationalDbShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-blue-500 text-blue-500"}
          `}
        >
          <img src={`/icons/vendors/${meta.vendor}.svg`} className="w-8 h-8" alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
          <span className="text-xs text-white mt-1">{meta.label || "Relational DB"}</span>
  ...
  indicator(shape: RelationalDbShapeType) {
