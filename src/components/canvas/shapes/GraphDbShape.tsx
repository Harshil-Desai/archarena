...
export type GraphDbShapeType = TLShape<"graphDb">;

function LabelEditor({ shape }: { shape: GraphDbShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-pink-400 rounded-lg
                 outline-none z-10"
...
export class GraphDbShapeUtil extends ShapeUtil<GraphDbShapeType> {
  static override type = "graphDb" as const;

  getDefaultProps(): GraphDbShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "neo4j", category: "graphDb", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: GraphDbShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: GraphDbShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-pink-400 text-pink-400"}
          `}
        >
          <img src={`/icons/vendors/${meta.vendor}.svg`} className="w-8 h-8" alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
          <span className="text-xs text-white mt-1">{meta.label || "Graph DB"}</span>
  ...
  indicator(shape: GraphDbShapeType) {
