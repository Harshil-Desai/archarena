...
export type ObjectStorageShapeType = TLShape<"objectStorage">;

function LabelEditor({ shape }: { shape: ObjectStorageShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-emerald-400 rounded-lg
                 outline-none z-10"
...
export class ObjectStorageShapeUtil extends ShapeUtil<ObjectStorageShapeType> {
  static override type = "objectStorage" as const;

  getDefaultProps(): ObjectStorageShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "s3", category: "objectStorage", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: ObjectStorageShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: ObjectStorageShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-emerald-400 text-emerald-400"}
          `}
        >
          <img src={`/icons/vendors/${meta.vendor}.svg`} className="w-8 h-8" alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
          <span className="text-xs text-white mt-1">{meta.label || "Object Storage"}</span>
  ...
  indicator(shape: ObjectStorageShapeType) {
