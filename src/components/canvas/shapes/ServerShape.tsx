...
export type ServerShapeType = TLShape<"server">;

function LabelEditor({ shape }: { shape: ServerShapeType }) {
...
export class ServerShapeUtil extends ShapeUtil<ServerShapeType> {
  static override type = "server" as const;

  getDefaultProps(): ServerShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "node", category: "server", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: ServerShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: ServerShapeType) {
  ...
  indicator(shape: ServerShapeType) {
