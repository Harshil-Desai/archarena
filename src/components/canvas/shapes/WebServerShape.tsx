...
export type WebServerShapeType = TLShape<"webServer">;

function LabelEditor({ shape }: { shape: WebServerShapeType }) {
...
export class WebServerShapeUtil extends ShapeUtil<WebServerShapeType> {
  static override type = "webServer" as const;

  getDefaultProps(): WebServerShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "nginx", category: "webServer", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: WebServerShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: WebServerShapeType) {
    ...
    return (
      <HTMLContainer>
        ...
        >
          ...
          <span className="text-xs text-white mt-1">{meta.label || "Web Server"}</span>
  ...
  indicator(shape: WebServerShapeType) {
