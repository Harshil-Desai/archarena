...
export type LoadBalancerShapeType = TLShape<"loadBalancer">;

function LabelEditor({ shape }: { shape: LoadBalancerShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-teal-400 rounded-lg
                 outline-none z-10"
...
export class LoadBalancerShapeUtil extends ShapeUtil<LoadBalancerShapeType> {
  static override type = "loadBalancer" as const;

  getDefaultProps(): LoadBalancerShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "loadbalancer", category: "loadBalancer", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: LoadBalancerShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: LoadBalancerShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-teal-400 text-teal-400"}
          `}
        >
          <img src={`/icons/vendors/${meta.vendor}.svg`} className="w-8 h-8" alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
          <span className="text-xs text-white mt-1">{meta.label || "Load Balancer"}</span>
  ...
  indicator(shape: LoadBalancerShapeType) {
