...
export type ClientShapeType = TLShape<"client">;

function LabelEditor({ shape }: { shape: ClientShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-cyan-400 rounded-lg
                 outline-none z-10"
...
export class ClientShapeUtil extends ShapeUtil<ClientShapeType> {
  static override type = "client" as const;

  getDefaultProps(): ClientShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "web", category: "client", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: ClientShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: ClientShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-cyan-400 text-cyan-400"}
          `}
        >
  ...
  indicator(shape: ClientShapeType) {
