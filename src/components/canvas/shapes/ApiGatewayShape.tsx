...
export type ApiGatewayShapeType = TLShape<"apiGateway">;

function LabelEditor({ shape }: { shape: ApiGatewayShapeType }) {
  ...
  return (
    <input
      ...
      className="absolute inset-0 w-full h-full bg-transparent text-white pt-10
                 text-xs text-center border-2 border-indigo-400 rounded-lg
                 outline-none z-10"
...
export class ApiGatewayShapeUtil extends ShapeUtil<ApiGatewayShapeType> {
  static override type = "apiGateway" as const;

  getDefaultProps(): ApiGatewayShapeType["props"] {
    return {
      w: 120,
      h: 80,
      meta: { vendor: "kong", category: "apiGateway", label: "", isLabeled: true },
    };
  }

  getGeometry(shape: ApiGatewayShapeType, _opts?: TLGeometryOpts): Geometry2d {
  ...
  component(shape: ApiGatewayShapeType) {
      ...
      <HTMLContainer>
        <div
          className={`
            ...
            ${isUnlabeled ? "border-red-500 stroke-red-500 text-red-500" : "border-indigo-400 text-indigo-400"}
          `}
        >
          <img src={`/icons/vendors/${meta.vendor}.svg`} className="w-8 h-8" alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
          <span className="text-xs text-white mt-1">{meta.label || "API Gateway"}</span>
  ...
  indicator(shape: ApiGatewayShapeType) {
